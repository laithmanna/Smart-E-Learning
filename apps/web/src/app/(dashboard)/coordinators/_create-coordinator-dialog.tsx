...
export function CreateCoordinatorDialog({ open, onClose, onCreated }: Props) {
  const t = useT();
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const created = await api<Coordinator>('/coordinators', {
        method: 'POST',
        body: {
          name,
          email,
  ...
  return (
    <Dialog
      ...
      title={t('userMgmt.coordinatorCreate')}
      description={t('userMgmt.coordinatorCreateDesc')}
    >
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? t('common.creating') : t('userMgmt.coordinatorCreating')}
          </Button>
        </div>
      </form>
    </Dialog>
